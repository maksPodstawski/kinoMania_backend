...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE users SET role = 'ROLE_VIP' WHERE user_id = :id", nativeQuery = true)
    void setVipRole(@Param("id") Long id);

    @Transactional
    @Modifying
    @Query(value = "UPDATE users SET vip_status = true WHERE user_id = :id", nativeQuery = true)
    void setVipStatus(@Param("id") Long id);
}
